...
NeighborRule::NeighborRule(const int &num_vertices)
    : Rule(0.5f), m_last_point(-1), m_num_vertices(num_vertices){};

NeighborRule::NeighborRule(const int &num_vertices, const double &jump_factor)
    : Rule(jump_factor), m_last_point(-1), m_num_vertices(num_vertices){};

bool NeighborRule::is_fullfilled(int dice)
{
    bool success = false;
...
double NeighborRule::get_jump_factor() const { return Rule::get_jump_factor(); };